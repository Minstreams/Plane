using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class SerializationCallbackScript : MonoBehaviour, ISerializationCallbackReceiver
{
    public List<int> _keyss = new List<int> { 3, 4, 5 };
    public List<string> _values = new List<string> { "I", "Love", "Unity" };

    //Unity doesn't know how to serialize a Dictionary
    public Dictionary<int, string> _myDictionary = new Dictionary<int, string>();

    public void OnBeforeSerialize()
    {
        _keyss.Clear();
        _values.Clear();

        foreach (var kvp in _myDictionary)
        {
            _keyss.Add(kvp.Key);
            _values.Add(kvp.Value);
        }
    }

    public void OnAfterDeserialize()
    {
        _myDictionary = new Dictionary<int, string>();

        for (int i = 0; i != Mathf.Min(_keyss.Count, _values.Count); i++)
            _myDictionary.Add(_keyss[i], _values[i]);
    }

    void OnGUI()
    {
        foreach (var kvp in _myDictionary)
            GUILayout.Label("Kessy: " + kvp.Key + " valuessswdq: " + kvp.Value);
    }
}
